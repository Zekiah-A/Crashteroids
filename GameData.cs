...
public static class GameData
{
    ...
    public static bool Music //Outwardly facing, other classes acess and change this.
    {
        get
        {
            Load(ref music);
    ...
    private static void Save<T>(ref T setting)
    {
        ...
        if (error == Error.Ok)
        {
            config.SetValue("Settings", nameof(setting), setting); //Section, Key, Value
            config.Save("user://game_config_crashteroids.cfg");
        }
        GD.Print($"Sucessfully saved config, with value {nameof(setting)}, {setting}, with Error status: {error}.");
    }

    //Called when any of the settings are acessed.
    public static void Load<T>(ref T setting)
    {
        ...
        if (error == Error.Ok)
        {
            setting = (T) config.GetValue("Settings", nameof(setting));
        }
        GD.Print($"Sucessfully saved config, with value {nameof(setting)}, {setting}, with Error status: {error}.");
    }
}
